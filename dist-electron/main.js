...
async function exampleAttendanceGeneration() {
  const exampleData = {
    // Use Omit to reflect removed properties
    ...
    logoPath: path.join(process.env.VITE_PUBLIC || "public", "image.png")
  ...
  try {
    const outputFile = await generateAttendanceSheet({
      outputFile: `./AttendanceSheet_${(/* @__PURE__ */ new Date()).toISOString().replace(/[T:.-]/g, "").slice(0, 14)}.pdf`,
      data: exampleData
      // Cast back to AttendanceData for the function call
    });
